...
fn main() {
    ...
    let intersections = path.intersections_with(&path_2);

    let steps: Vec<((i32, i32), i32)> = intersections
        .iter()
...
impl Path {
    pub fn push(&mut self, dir: Direction) {
        ...
        match dir {
            Direction::Up(delta_y) => for i in 0..delta_y as i32 {
                self.path.push((
                    starting_x,
                    starting_y + (i+1)
                ));
            }
            Direction::Down(delta_y) => for i in 0..delta_y as i32 {
                self.path.push((
                    starting_x,
                    starting_y - (i+1),
                ));
            }
            Direction::Right(delta_x) => for i in 0..delta_x as i32 {
                self.path.push((
                    starting_x + (i+1),
                    starting_y,
                ))
            }
            Direction::Left(delta_x) => for i in 0..delta_x as i32 {
                self.path.push((
                    starting_x - (i+1),
                    starting_y,
               ));
    ...
    pub fn intersections_with(&self, other: &Path) -> Vec<(i32, i32)> {
        println!(
             "Calculating intersections: iterations: {} * {} = {}",
             self.path.len(),
             other.path.len(),
    ...
    pub fn steps_until(&self, point: (i32, i32)) -> i32 {
        let mut counter = 0;
...
impl Direction {
    fn from(string: &str) -> Self {
        ...
        let number = number_text.parse::<u32>().expect("Can't parse number in `Direction::from`");

        match string.chars().nth(0).expect("No chars in `Direction::from` string") {
